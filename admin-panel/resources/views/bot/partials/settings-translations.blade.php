<div x-show="activeTab === 'translations'" x-cloak class="grid gap-6 xl:grid-cols-4" x-transition>
                <aside class="space-y-4 xl:col-span-1">
                    <section class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-200 dark:bg-white">
                        <div class="mb-3">
                        ...
                        <div class="space-y-2">
                            @foreach ($translationGroupDefinitions as $group)
                                ...
                                >
                                    ...
                                    <span class="rounded-full bg-white px-2 py-0.5 text-[11px] font-semibold text-gray-500 shadow-sm dark:bg-white dark:text-gray-300">{{ $translationGroupCounts[$group['key']] ?? 0 }}</span>
                                </button>
                            @endforeach
                        </div>
                    </section>
                ...
                <section class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-200 dark:bg-white xl:col-span-3">
                    <div class="flex flex-col gap-3 border-b border-gray-200 p-4 dark:border-gray-200 lg:flex-row lg:items-center lg:justify-between">
                        <div>
                        ...
                        <div class="w-full lg:max-w-sm">
                            <div class="relative">
                                <input
                                    ...
                                    class="w-full rounded-xl border-gray-300 bg-gray-50 pl-10 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 dark:border-gray-300 dark:bg-gray-100 dark:text-gray-100"
                                >
                    ...
                    <div class="grid min-h-[560px] grid-cols-1 gap-0 lg:grid-cols-3">
                        <div class="border-b border-gray-200 bg-gray-50/60 dark:border-gray-200 dark:bg-gray-100/40 lg:border-b-0 lg:border-r">

                            <div class="max-h-[560px] divide-y divide-gray-100 overflow-y-auto dark:divide-gray-100">
                                @foreach ($translationKeys as $keyItem)
                                    <div
                                        ...
                                        :class="selectedKeyId === {{ $keyItem->id }} ? 'bg-white border-l-4 border-emerald-500 dark:bg-white' : 'cursor-pointer border-l-4 border-transparent hover:bg-white/70 dark:hover:bg-white/70'"
                                        class="px-4 py-3 transition"
                                    >
                                        <div class="flex items-center justify-between gap-3">
                                            <span class="truncate rounded-full bg-gray-200 px-2 py-0.5 text-[10px] font-semibold text-gray-700 dark:bg-gray-200 dark:text-gray-200 flex items-center gap-1">
                        ...
                        <div class="lg:col-span-2 p-5">
                            @foreach ($translationKeys as $keyItem)
                                <div x-show="selectedKeyId === {{ $keyItem->id }}" x-cloak class="h-full" x-transition>
                                    <div class="mb-4 flex flex-wrap items-start justify-between gap-3 border-b border-gray-100 pb-4 dark:border-gray-200">
                                        <div>
                                    ...
                                    <form method="POST" action="{{ route('bot.translations.update', $keyItem) }}" class="space-y-4" @submit.prevent="submitForm">
                                        ...
                                        <div class="grid gap-4">
                                            <div>
                                                ...
                                                <textarea name="values[es]" rows="4" class="w-full rounded-2xl border-gray-300 bg-gray-50 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 dark:border-gray-300 dark:bg-gray-100 dark:text-gray-100" required @input="markDirty" @input="markDirty">{{ $keyItem->translations->where('lang', 'es')->first()?->value }}</textarea>
                                            </div>
                                            <div>
                                                <label class="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Quechua (QU)</label>
                                                <textarea name="values[qu]" rows="4" class="w-full rounded-2xl border-gray-300 bg-gray-50 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 dark:border-gray-300 dark:bg-gray-100 dark:text-gray-100" required @input="markDirty" @input="markDirty">{{ $keyItem->translations->where('lang', 'qu')->first()?->value }}</textarea>
                                            </div>
                                            <div>
                                                ...
                                                <textarea name="values[ash]" rows="4" class="w-full rounded-2xl border-gray-300 bg-gray-50 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500 dark:border-gray-300 dark:bg-gray-100 dark:text-gray-100" required @input="markDirty" @input="markDirty">{{ $keyItem->translations->where('lang', 'ash')->first()?->value }}</textarea>
                                            </div>
                                        </div>

                                        <div class="flex items-center justify-between gap-3 border-t border-gray-100 pt-4 dark:border-gray-200">
                            ...
                            <div x-show="selectedKeyId === null" x-cloak class="flex h-full min-h-[500px] items-center justify-center text-center">
                                <div class="max-w-sm">
                                    <svg class="mx-auto h-12 w-12 text-gray-300 dark:text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
